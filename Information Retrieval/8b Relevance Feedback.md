#exam The probabilistic model of Information Retrieval makes use of two probabilities relating to query terms. These are 𝑃(𝑘i|𝑅) (the probability that a relevant document will contain the term ki) and 𝑃(𝑘i|𝑅') (the probability that a non-relevant document will contain the term ki). However, these probabilities cannot be calculated directly and must be estimated. 
